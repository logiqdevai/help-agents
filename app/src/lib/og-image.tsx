...
/** Brand share card: off-white canvas, pastel orbs, ink type (DESIGN.MD). */
export function renderOgImage({ headline, subline }: OgImageProps) {
    ...
    (
      ...
      >
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 28 }}>
          <div style={{ fontSize: 88, lineHeight: 1.04, letterSpacing: -3, maxWidth: 900 }}>{headline}</div>
          <div style={{ fontSize: 34, color: "#4e4e4e" }}>{subline}</div>
        </div>
      </div>
    ),
    { ...OgImageSize },
