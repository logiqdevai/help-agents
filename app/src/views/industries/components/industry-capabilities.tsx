...
export function IndustryCapabilities({ industry }: { industry: IndustryContent }) {
  const { capabilities } = industry;
  return (
          ...
          {capabilities.items.map((item) => (
            <li key={item.title} className="bg-canvas p-7 lg:p-8">
              <div className="flex items-center justify-between gap-4">
                ...
                <span className="rounded-full border border-hairline-strong px-2.5 py-0.5 text-[13px] text-muted-ink">
                  {item.channel}
                </span>
              </div>
