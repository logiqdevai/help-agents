...
import { AgentUsage, OutcomeCount, UsageTotals } from './interfaces/dashboard.interface';
...
export class AnalyticsService {
  ...
  async getUsage(ctx: CompanyContextData, query: UsageQueryType) {
    ...
    const [totals, agents] = await Promise.all([this.stats.totals(where), this.perAgent(where)]);
  ...
  private async perAgent(where: Parameters<CallStatsService['totals']>[0]): Promise<AgentUsage[]> {
    const [callRows, successRows, outcomeRows] = await Promise.all([
      ...
      this.prisma.call.groupBy({
        by: ['agent_uuid', 'outcome_key', 'outcome_label'],
        where,
        _count: { _all: true },
      }),
    ]);
    ...
    const agents = await this.prisma.agent.findMany({
      where: { id: { in: callRows.map((r) => r.agent_uuid) } },
      select: { id: true, name: true },
    });

    return callRows
      .map((row) => {
        const successful = successRows.find((s) => s.agent_uuid === row.agent_uuid)?._count._all ?? 0;
        const calls = row._count._all;
        const outcomes: OutcomeCount[] = outcomeRows
          .filter((o) => o.agent_uuid === row.agent_uuid)
          .map((o) => ({ key: o.outcome_key, label: o.outcome_label, count: o._count._all }))
          .sort((a, b) => b.count - a.count);

        return {
          agent: {
            id: row.agent_uuid,
            name: agents.find((a) => a.id === row.agent_uuid)?.name ?? 'Unknown agent',
          },
          calls_made: calls,
