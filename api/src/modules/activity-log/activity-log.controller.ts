...
export class ActivityLogController {
  ...
  @ApiQuery({ name: 'entity_type', required: false, description: 'One or more types, comma separated' })
  @ApiQuery({ name: 'entity_uuid', required: false })
  @ApiQuery({ name: 'user_uuid', required: false })
  @ApiQuery({ name: 'actor_type', required: false, enum: ['USER', 'SYSTEM', 'AGENT', 'PROVIDER'] })
  @ApiQuery({ name: 'search', required: false, description: 'Matches the action or the actor name / email' })
